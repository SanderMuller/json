...
- `SanderMuller\Json\Exceptions\UnexpectedJsonShapeException` — thrown when JSON is valid but decodes to a different shape than the called method asserts. Extends the native `JsonException`.

### Methods

All `Json` methods are static and force `JSON_THROW_ON_ERROR`. `$flags` is OR-ed onto that; `$depth` is a passthrough.
...
- `UnexpectedJsonShapeException::expected(string $expected, mixed $actual): self`
...
Anything under `SanderMuller\Json\Internal`, anything marked `@internal` in PHPDoc, anything in `src/Support/` and `src/Concerns/`.
